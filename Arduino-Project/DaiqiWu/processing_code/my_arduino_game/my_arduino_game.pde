...
void setup() {
  ...
  screen = 0;
  frameRate(60);
  noStroke(); // pre-set condition to ensure all shapes are drawn without strokes on the border
  //port.bufferUntil('\n');
}
...
void set_color_mode() {
  ...
  //print(light); print(' '); // for debug purposes
  ...
  //print(color_mode); print(' '); // for debug purposes
  //if (bbs == "1023") color_mode = color_mode ^ color_mode;
  // changing colour mode if pressed everytime. xor would do the trick
}
...
void set_ball_size() {
  ...
  //println(b.s); // for debug purposes
}
...
void border_patrol() {
  ...
  //if (b.x - (b.s / 2) < 0) bounce_surface(0, 2); // bounce on the left side of the screen, use this to aviod dying / will continue forever
...
void move_holder() {
  ...
  //} else {    
  // I did try to use Arduino to control this game. This didn't work out. Leave it here just in case I have a joystick to control it.
      h.x = mouseX;
      h.y_prev = h.y;
      h.y = mouseY;
  //}
  ...
  //}
  // Uncomment this bit if you want to use keyboard instead of mouse
}
...
void bounce_vertical(float accl) {
  if ((b.x + (b.s / 2) > h.x - (h.w / 2)) && (b.x - (b.s / 2) < h.x + (h.w / 2)) && (dist(b.x, b.y, b.x, h.y) <= (b.s / 2) + abs(accl)) &&
   (b.y + (b.s / 2) > h.y) && (b.y - (b.s / 2) < h.y + h.w / 2)) { // check the ball if need to bounce on the holder
  ...
  if ((b.x + (b.s / 2) > h.x - (h.w / 2)) && (b.x - (b.s / 2) < h.x + (h.w / 2)) && (dist(b.x, b.y, b.x, h.y) <= (b.s / 2) + abs(accl)) &&
  (b.y + (b.s / 2) > h.y + h.w / 2) && (b.y - (b.s / 2) < h.y + h.w)) { // check the ball if need to bounce under the holder 
...
} // give horizontal speed just like Pong game!
